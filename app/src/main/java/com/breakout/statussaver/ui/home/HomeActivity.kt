...
class HomeActivity : AppCompatActivity() {
    ...
    private val savedAdapter = StatusAdapter({ openPreview(it) }, {})
    private var curTab = 1

    private val folderPickerLauncher = registerForActivityResult(androidx.activity.result.contract.ActivityResultContracts.OpenDocumentTree()) { uri: Uri? ->
        if (uri != null) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupToolbar(); setupTabs(); setupRVs(); setupSwipe(); setupBanner()
        AdManager.preloadAppOpenAd(this)

        if (PermissionUtils.hasStoragePermission(this)) { showAppOpenThenLoad() } else { requestPermissions() }
    ...
    private fun requestPermissions() { ActivityCompat.requestPermissions(this, PermissionUtils.getRequiredPermissions(), PERMISSION_REQUEST_CODE) }
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        super.onRequestPermissionsResult(requestCode, permissions, grantResults)
        if (requestCode == PERMISSION_REQUEST_CODE) {
            if (grantResults.isNotEmpty() && grantResults.all { it == PackageManager.PERMISSION_GRANTED }) showAppOpenThenLoad()
            else showPermissionDeniedDialog()
        }
    }

    private fun showPermissionDeniedDialog() {
        AlertDialog.Builder(this).setTitle("Storage Permission Required").setMessage("The app needs access to storage to view statuses.")
            .setPositiveButton("Grant") { _, _ -> requestPermissions() }.setNegativeButton("Exit") { _, _ -> finishAffinity() }.setCancelable(false).show()
    }

    private fun setupToolbar() { setSupportActionBar(binding.toolbar); supportActionBar?.title = "Status Saver images & videos" }
    ...
    private fun handleDelete() {
        // STRICT: ONLY allow deleting from the Saved tab (curTab == 2)
        if (curTab != 2) {
        ...
        AlertDialog.Builder(this).setTitle("Delete ${itemsToDelete.size} saved items?").setMessage("This will permanently delete the saved files from your device.")
            .setPositiveButton("Delete") { _, _ ->
                var c = 0; for (d in itemsToDelete) { try { if (d.delete()) c++ else { contentResolver.delete(d.uri, null, null); c++ } } catch (e: Exception) {} }
                savedAdapter.clearSelection(); loadAllStatuses(); Toast.makeText(this, "Deleted $c saved items", Toast.LENGTH_SHORT).show()
            }.setNegativeButton("Cancel") { _, _ -> savedAdapter.clearSelection() }.show()
    }
    ...
    private fun loadAllStatuses() {
        ...
        Thread {
            ...
            } catch (e: Throwable) { needsPicker = true } finally {
                mainHandler.post {
    ...
    private fun showFolderPickerDialog() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("OK") { _, _ ->
                try { folderPickerLauncher.launch(android.provider.DocumentsContract.buildDocumentUri("com.android.externalstorage.documents", "primary:Android/media")) } catch (e: Exception) { folderPickerLauncher.launch(null) }
            }
    ...
    private fun setupTabs() {
        ...
        binding.tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) { curTab = tab.position; imgAdapter.clearSelection(); vidAdapter.clearSelection(); savedAdapter.clearSelection(); showTab(tab.position) }
    ...
    private fun showTab(p: Int) {
        binding.recyclerViewImages.visibility = View.GONE; binding.recyclerViewVideos.visibility = View.GONE; binding.recyclerViewSaved.visibility = View.GONE; binding.layoutEmpty.root.visibility = View.GONE
        when (p) {
    ...
    private fun showEmpty(msg: String) { binding.layoutEmpty.tvEmptyMessage.text = msg; binding.layoutEmpty.root.visibility = View.VISIBLE }
    private fun setupRVs() {
        val s = 3
        binding.recyclerViewImages.layoutManager = GridLayoutManager(this, s); binding.recyclerViewImages.adapter = imgAdapter
        binding.recyclerViewVideos.layoutManager = GridLayoutManager(this, s); binding.recyclerViewVideos.adapter = vidAdapter
        binding.recyclerViewSaved.layoutManager = GridLayoutManager(this, s); binding.recyclerViewSaved.adapter = savedAdapter
    }
    private fun setupSwipe() { binding.swipeRefreshLayout.setColorSchemeResources(R.color.primary, R.color.primary_dark); binding.swipeRefreshLayout.setOnRefreshListener { binding.swipeRefreshLayout.isRefreshing = false; loadAllStatuses() } }
    private fun setupBanner() { try { binding.adView.adUnitId = AdManager.BANNER_ID; binding.adView.loadAd(com.google.android.gms.ads.AdRequest.Builder().build()) } catch (e: Exception) { binding.adView.visibility = View.GONE } }
    private fun openPreview(doc: DocumentFile) { if (AdManager.canShowOnPreview()) AdManager.showInterstitial(this) { doPreview(doc) } else doPreview(doc) }
